...
trait SpanInspection {
  ...
  class Inspector(span: Span) {
    ...
    def toFinished(at: Instant): Span.Finished =
      if (span.isInstanceOf[Span.Local])
        Reflection.invoke[Span.Local, Span.Finished](span.asInstanceOf[Span.Local], "toFinishedSpan", (classOf[Instant], at), (classOf[TagSet], metricTags()))
      else
  ...
  trait Syntax {
    ...
    implicit def spanToRichSpan(span: Span): RichSpan =
      new RichSpan {
