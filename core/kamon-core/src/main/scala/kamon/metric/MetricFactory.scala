...
class MetricFactory private (
  ...
  def counter(
    ...
    val builder = (metric: BaseMetric[Counter, Metric.Settings.ForValueInstrument, Long], tags: TagSet) => new Counter.LongAdder(metric, tags)

    new BaseMetric[Counter, Metric.Settings.ForValueInstrument, Long](name, metricDescription, metricSettings, builder, scheduler) with Metric.Counter {
  ...
  def gauge(
    ...
    val builder = (metric: BaseMetric[Gauge, Metric.Settings.ForValueInstrument, Double], tags: TagSet) => new Gauge.Volatile(metric, tags)

    new BaseMetric[Gauge, Metric.Settings.ForValueInstrument, Double](name, metricDescription, metricSettings, builder, scheduler) with Metric.Gauge {
  ...
  def histogram(
    ...
    new BaseMetric[Histogram, Metric.Settings.ForDistributionInstrument, Distribution](name, metricDescription, metricSettings, builder, scheduler)
  ...
  def timer(
    ...
    new BaseMetric[Timer, Metric.Settings.ForDistributionInstrument, Distribution](name, metricDescription, metricSettings, builder, scheduler)
      with Metric.Timer {
  ...
  def rangeSampler(
    ...
    new BaseMetric[RangeSampler, Metric.Settings.ForDistributionInstrument, Distribution](name, metricDescription, metricSettings, builder, scheduler)
...
object MetricFactory {
  ...
  def from(config: Config, scheduler: ScheduledExecutorService, clock: Clock): MetricFactory = {
    ...
    val customSettings = factoryConfig
      .getConfig("custom-settings")
      .configurations
  ...
  private def nonEmptySection(entry: (String, Config)): Boolean =
    entry match {
