...
class DatadogAPIMetricsSender extends Actor with ActorLogging {
  ...
  val client = new DefaultAsyncHttpClient(new DefaultAsyncHttpClientConfig.Builder()
    .setConnectTimeout(config.getDuration("http.connect-timeout").toMillis.toInt)
    .setReadTimeout(config.getDuration("http.read-timeout").toMillis.toInt)
    .setRequestTimeout(config.getDuration("http.request-timeout").toMillis.toInt)
    .build)
    ...
    val series:String = (for {
      ...
      metricSnapshot match {
        case hs: Histogram.Snapshot ⇒
          val min = hs.min
          val max = hs.max
          val cnt = hs.numberOfMeasurements
          val sum = hs.sum
          val p95 = hs.percentile(0.95)
