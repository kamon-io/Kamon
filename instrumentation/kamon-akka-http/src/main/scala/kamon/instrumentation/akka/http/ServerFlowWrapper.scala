...
object ServerFlowWrapper {
          ...
          setHandler(
            ...
            new InHandler {
              override def onPush(): Unit = {
                ...
                val requestHandler = httpServerInstrumentation
                  .createHandler(toRequest(request), deferSamplingDecision = true)
                  .requestReceived()
                ...
                Kamon.storeContext(
                  requestHandler.context.withEntry(
  ...
  def defaultOperationName(listenPort: Int): String =
    _defaultOperationNames
      .getOrElseUpdate(listenPort, _serverInstrumentations.get(listenPort).map(_.settings.defaultOperationName).getOrElse("http.server.request"))
