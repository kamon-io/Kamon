...
object JdbcInstrumentation extends LoggingSupport {
  ...
  private def readSettings(config: Config): Settings = {
    ...
    val slowStatementProcessors = jdbcConfig
      .getStringList("statements.slow.processors")
      .asScala
      .map(fqcn => ClassLoading.createInstance[SlowStatementProcessor](fqcn))
      .toList

    val failedStatementProcessors = jdbcConfig
      .getStringList("statements.failed.processors")
      .asScala
