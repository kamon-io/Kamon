...
object NewRelicSpanConverter {
  ...
  def convertSpan(kamonSpan: Span.Finished): NewRelicSpan = {
    ...
    val builder = NewRelicSpan
      .builder(kamonSpan.id.string)
  ...
  private def buildAttributes(kamonSpan: Span.Finished) = {
    ...
    if (kamonSpan.kind == Span.Kind.Client) {
      val remoteEndpoint = Endpoint(getStringTag(kamonSpan, PeerKeys.IPv4), getStringTag(kamonSpan, PeerKeys.IPv6), getLongTag(kamonSpan, PeerKeys.Port).toInt)
