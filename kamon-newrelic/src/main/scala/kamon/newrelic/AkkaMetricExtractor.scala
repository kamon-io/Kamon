...
object AkkaMetricExtractor extends MetricExtractor {
  ...
  protected[newrelic] def normalize(name: String) = name.replace('/', '\\').replaceAll("""[\]\[\|\*]""", "_")

  protected[newrelic]type Operation = String ⇒ Option[InstrumentSnapshot]

  protected[newrelic] def logMetrics(nameAndGetterPairs: EntitySnapshot ⇒ Seq[(String, Operation)])(actorName: String, actorSnapshot: EntitySnapshot, category: String): Iterable[Metric] =
    for {
      ...
      entityCategory = camelize(category)
      entityName = normalize(actorName)
    } yield Metric(metric, key.unitOfMeasurement, s"Custom/$entityCategory/$entityName/$metricName", None)
}
