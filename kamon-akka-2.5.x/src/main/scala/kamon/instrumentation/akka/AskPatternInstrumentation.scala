...
import kamon.instrumentation.akka.common.AkkaPrivateAccess
...
class AskPatternInstrumentation extends InstrumentationBuilder {
  ...
  onType("akka.pattern.AskableActorRef$")
    .advise(method("$qmark$extension"), AskPatternInstrumentation.AskMethodAdvice)
