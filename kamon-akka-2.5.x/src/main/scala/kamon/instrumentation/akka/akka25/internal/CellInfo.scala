...
case class CellInfo(path: String, isRouter: Boolean, isRoutee: Boolean, isTracked: Boolean, trackingGroups: Seq[String],
    actorCellCreation: Boolean, systemName: String, dispatcherName: String, isTraced: Boolean, startsTrace: Boolean,
    actorOrRouterClass: Class[_], routeeClass: Option[Class[_]], actorName: String)
...
object CellInfo {
  ...
  def cellInfoFor(cell: Cell, system: ActorSystem, ref: ActorRef, parent: ActorRef, actorCellCreation: Boolean): CellInfo = {
    ...
    val filterName = if (isRouter || isRoutee) AkkaInstrumentation.TrackRouterFilterName else AkkaInstrumentation.TrackActorFilterName
    val isTracked = !isRootSupervisor && Kamon.filter(filterName).accept(fullPath)
    val isTraced = Kamon.filter(AkkaInstrumentation.TraceActorFilterName).accept(fullPath)
    val startsTrace = Kamon.filter(AkkaInstrumentation.StartTraceActorFilterName).accept(fullPath)
    val trackingGroups = if(isRootSupervisor) List() else AkkaInstrumentation.matchingActorGroups(fullPath)
    ...
    CellInfo(fullPath, isRouter, isRoutee, isTracked, trackingGroups, actorCellCreation, system.name, dispatcherName,
      isTraced, startsTrace, actorOrRouterClass, routeeClass, actorName)
  }
}
