...
package kamon.instrumentation.akka.akka25.advisor
...
object PublishMethodAdvisor {
  ...
  def onExit(@This stream:HasSystem, @Argument(0) event: AnyRef):Unit = event match {
    case dl: DeadLetter => AkkaMetrics.forSystem(stream.system.name).deadLetters.increment()
    case um: UnhandledMessage => AkkaMetrics.forSystem(stream.system.name).unhandledMessages.increment()
    case _ => ()
  }
}