...
import kamon.instrumentation.akka.akka25.advisor.{ConstructorAdvisor, PublishMethodAdvisor}
import kamon.instrumentation.akka.akka25.mixin.HasSystemMixin
...
class DeadLettersInstrumentation extends InstrumentationBuilder {
  ...
  onType("akka.event.EventStream")
    ...
    .advise(isConstructor.and(takesArguments(2)), classOf[ConstructorAdvisor])
