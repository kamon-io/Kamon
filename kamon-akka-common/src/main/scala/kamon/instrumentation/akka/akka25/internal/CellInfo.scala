...
object CellInfo {
  ...
  def cellInfoFor(cell: Cell, system: ActorSystem, ref: ActorRef, parent: ActorRef, actorCellCreation: Boolean): CellInfo = {
    ...
    val trackedFilter = if (isRouter || isRoutee) Kamon.filter(TrackRouterFilterName) else Kamon.filter(TrackActorFilterName)
    val isTracked = !isRootSupervisor && trackedFilter.accept(fullPath)
    val isTraced = Kamon.filter(TraceActorFilterName).accept(fullPath)
    val startsTrace = Kamon.filter(StartTraceActorFilterName).accept(fullPath)
    val trackingGroups = if(isRootSupervisor) List() else {
      if(!isTracked && settings().autoGrouping) {
        if(!trackedFilter.excludes(fullPath)) {
      ...
      AkkaInstrumentation.matchingActorGroups(fullPath)
    }
